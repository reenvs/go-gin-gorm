...
var blockURLList = [...]string{"/ims/installation/count"}
...
func (p *Prometheus) registerMetrics(subsystem string) {
	...
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			logger.Errorf("err:%v,%s could not be registered in Prometheus", err, metricDef.Name)
		}
		switch metricDef {
		case reqCnt:
...
func isURLBlock(url string) bool {
	var ret = false
	for _, v := range blockURLList {
